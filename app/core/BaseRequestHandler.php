...
class BaseRequestHandler
{
	...
	public function __construct()
	{
		...
		$this->topicId = $this->telegram->getTopicId();
		if (!empty($this->topicId)) $this->chat .= "_" . $this->topicId;
	}
	...
	protected function sendTo($chat, $text, $buttons = [], $parseMode = 'HTML', $toTopic = true): string
	{
		return $this->telegram->sendMessage($chat, $text, $buttons, $parseMode, true, $toTopic);
	}

	protected function send($text, $buttons = [], $parseMode = 'HTML', $toTopic = true): string
	{
		return $this->sendTo($this->chat, $text, $buttons, $parseMode, $toTopic);
	}
	...
	public function pinMessage($messageId, $chat = null)
	{
		$this->telegram->pinChatMessage(($chat ?? $this->chat), $messageId);
	}

	public function unpinMessage($messageId, $chat = null)
	{
		$this->telegram->unpinChatMessage(($chat ?? $this->chat), $messageId);
	}
}
